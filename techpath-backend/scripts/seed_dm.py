#!/usr/bin/env python3
"""Seed training modules for Digital Marketing with Gen AI via the production API.
...
PROGRAM_ID = 4
ASSETS_DIR = Path(r"D:\project\techpath\techpath-2026\syllabus\digital marketing with gen ai\assests")
DELAY = 0.15
...
def build_payload(filepath: Path, ai_meta: dict):
    ...
    if atype in ["markdown", "notes", "cheat_sheet", "code_snippet"]:
        ...
        if atype == "code_snippet":
            # Best guess from extension, generate_dm_metadata didn't store language explicitly in meta
        ...
        try:
            data = json.loads(content)
            payload["objective"] = data.get("objective", "")
            payload["steps"] = data.get("steps", [])
            payload["starter_code"] = data.get("starter_code")
            payload["expected_output"] = data.get("expected_output")
        except:
            pass
...
def seed():
    print("=" * 60)
    print("Seeding Digital Marketing via API (using metadata.json exclusively)")
    print("=" * 60)
